...
class My_loss1(nn.Module):
    ...
    def forward(self,forecast, y):
        return torch.mean(torch.pow((forecast[:,0] - y[:,1]), 2))+torch.mean(torch.pow((forecast[:,1]- y[:,2]), 2))
...
class My_loss6(nn.Module):#能下降
    ...
    def forward(self,forecast, y):
        kd=y[:,0]*forecast[:,0]        
        kb=y[:,0]*forecast[:,1] 
        return torch.mean(torch.abs(kd - y[:,1]))+torch.mean(torch.abs(kb- y[:,2]))
"""          
...
def hook(model,device,test_dataset):
    test_X = torch.tensor(test_dataset[:,0:(test_dataset.shape[1]-2)],dtype=torch.float32)
    test_set = TensorDataset(test_X)
    test_loader = DataLoader(test_set, batch_size=10000,shuffle=False)
    result=np.zeros((1,16))
    model.eval()
    weight_raw=np.zeros((16))
    for _data in test_loader:
        hf = _data[0].to(device)
        for name, module in model.named_children():
            if name == "seperate_modeling":
                kd_ratio_ensem,kb_ratio_ensem,x=module(hf)
            if name == "weight":
                weight = module(x)
                tmp=weight.detach().cpu().numpy()
                tmp=np.squeeze(tmp,2)
...
class My_loss_way2(nn.Module):#无法下降0.08
    def __init__(self):
        super().__init__()
...
class proposed_loss(nn.Module):#无法下降0.08
    def __init__(self):
        super().__init__()
